...
public class GooseEntityModel extends FlyingBirdEntityModel<GooseEntity> {
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        ModelPartData neck = body.addChild("neck", ModelPartBuilder.create().uv(0, 23).cuboid(-1.5F, -7.0F, -1.75F, 3.0F, 9.0F, 3.0F, new Dilation(-0.001F)), ModelTransform.pivot(0.0F, -4.5F, -1.75F));
    ...
    public void animateModel(GooseEntity goose, float limbAngle, float limbDistance, float tickDelta) {
        ...
        if(!goose.isFlying() && !goose.isInsideWaterOrBubbleColumn()) {
            this.animateMovement(GooseAnimations.WALKING, limbAngle, limbDistance, 4F, 4F);
        }
