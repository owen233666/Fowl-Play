...
public class BabyGooseModel extends GooseModel {
    ...
    public static LayerDefinition getTexturedModelData() {
        ...
        left_wing_open.addOrReplaceChild("left_wing_outer", CubeListBuilder.create().texOffs(24, 10).addBox(0.0F, 0.0F, 0.0F, 10.0F, 0.0F, 8.0F, new CubeDeformation(0.0F)), PartPose.offset(9.0F, -0.1F, -1.0F));
    ...
    public void prepareMobModel(GooseEntity goose, float limbAngle, float limbDistance, float tickDelta) {
        ...
        final AnimationDefinition walkingAnimation = domestic ? DomesticGooseAnimations.WALKING : GooseAnimations.WALKING;
        final AnimationDefinition standingAnimation = domestic ? DomesticGooseAnimations.STANDING : GooseAnimations.STANDING;
        final AnimationDefinition swimmingAnimation = domestic ? DomesticGooseAnimations.SWIMMING : GooseAnimations.SWIMMING;
        final AnimationDefinition glidingAnimation = domestic ? DomesticGooseAnimations.GLIDING : GooseAnimations.GLIDING;
        final AnimationDefinition flappingAnimation = domestic ? DomesticGooseAnimations.FLAPPING : GooseAnimations.FLAPPING;

        if(!goose.isFlying() && !goose.isInWaterOrBubble()) {
            this.animateWalk(walkingAnimation, limbAngle, limbDistance, 3F, 3F);
        }
        this.animate(goose.standingState, standingAnimation, ageInTicks);
        this.animate(goose.swimmingState, swimmingAnimation, ageInTicks);
