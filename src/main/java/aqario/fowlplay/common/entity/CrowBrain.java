...
public class CrowBrain {
    private static final ImmutableList<SensorType<? extends Sensor<? super CrowEntity>>> SENSORS = ImmutableList.of(
    ...
    public static Brain.Profile<CrowEntity> createProfile() {
        return Brain.createProfile(MEMORIES, SENSORS);
    }

    public static Brain<?> create(Brain<CrowEntity> brain) {
    ...
    public static void reset(CrowEntity crow) {
        Brain<CrowEntity> brain = crow.getBrain();
    ...
    private static void addCoreActivities(Brain<CrowEntity> brain) {
        brain.setTaskList(
            Activity.CORE,
            0,
            ImmutableList.of(
    ...
    private static void addIdleActivities(Brain<CrowEntity> brain) {
        brain.setTaskList(
            Activity.IDLE,
            ImmutableList.of(
                Pair.of(1, new BreedTask(FowlPlayEntityType.CROW, Birds.WALK_SPEED, 20)),
                Pair.of(2, WalkTowardClosestAdultTask.create(Birds.FOLLOW_ADULT_RANGE, Birds.WALK_SPEED)),
                Pair.of(3, LookAtMobTask.create(CrowBrain::isPlayerHoldingFood, 32.0F)),
                Pair.of(4, UpdateAttackTargetTask.create(crow -> !crow.isInsideWaterOrBubbleColumn(), CrowBrain::getAttackTarget)),
                ...
                Pair.of(
                    ...
                    new RandomTask<>(
                        ...
                        ImmutableList.of(
                            ...
                            Pair.of(FlightControlTask.startFlying(crow -> crow.isInsideWaterOrBubbleColumn() || crow.getRandom().nextFloat() < 0.3F), 1)
                        )
                    )
                )
            ),
    ...
    private static void addFlyActivities(Brain<CrowEntity> brain) {
        brain.setTaskList(
            FowlPlayActivities.FLY,
            ImmutableList.of(
                Pair.of(1, FlightControlTask.tryStopFlying(crow -> true)),
                Pair.of(2, UpdateAttackTargetTask.create(CrowBrain::getAttackTarget)),
    ...
    private static void addAvoidActivities(Brain<CrowEntity> brain) {
        brain.setTaskList(
            Activity.AVOID,
            10,
            ImmutableList.of(
                FlightControlTask.startFlying(crow -> true),
                MoveAwayFromTargetTask.entity(
                    MemoryModuleType.AVOID_TARGET,
                    crow -> crow.isFlying() ? Birds.FLY_SPEED : Birds.RUN_SPEED,
                    true
                ),
                CompositeTasks.makeRandomFollowTask(FowlPlayEntityType.CROW),
    ...
    private static void addPickupFoodActivities(Brain<CrowEntity> brain) {
        brain.setTaskList(
            FowlPlayActivities.PICK_UP,
            ImmutableList.of(
    ...
    private static void addFightActivities(Brain<CrowEntity> brain) {
        brain.setTaskList(
            Activity.FIGHT,
            0,
            ImmutableList.of(
    ...
    private static Optional<? extends LivingEntity> getAttackTarget(CrowEntity crow) {
        return LookTargetUtil.hasBreedTarget(crow) ? Optional.empty() : crow.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_ATTACKABLE);
    }

    public static void onAttacked(CrowEntity crow, LivingEntity attacker) {
        if (attacker instanceof CrowEntity) {
            return;
        }
        Brain<CrowEntity> brain = crow.getBrain();
        ...
        if (attacker instanceof PlayerEntity player) {
            ...
            crow.stopTrusting(player);
        }
        brain.remember(MemoryModuleType.AVOID_TARGET, attacker, 160L);
        alertOthers(crow, attacker);
    }

    protected static void alertOthers(CrowEntity crow, LivingEntity attacker) {
        getNearbyVisibleCrows(crow).forEach(other -> {
            if (attacker instanceof PlayerEntity player) {
                other.getBrain().remember(FowlPlayMemoryModuleType.CANNOT_PICKUP_FOOD, true, 1200L);
                ((CrowEntity) other).stopTrusting(player);
            }
            runAwayFrom((CrowEntity) other, attacker);
        });
    }

    protected static void runAwayFrom(CrowEntity crow, LivingEntity target) {
        crow.getBrain().forget(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE);
        crow.getBrain().remember(MemoryModuleType.AVOID_TARGET, target, 160L);
    }

    protected static List<? extends PassiveEntity> getNearbyVisibleCrows(CrowEntity crow) {
        return crow.getBrain().getOptionalRegisteredMemory(FowlPlayMemoryModuleType.NEAREST_VISIBLE_ADULTS).orElse(ImmutableList.of());
    }
